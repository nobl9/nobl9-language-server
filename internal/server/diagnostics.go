...
type diagnosticsObjectsProvider interface {
	GetObject(ctx context.Context, kind manifest.Kind, name, project string) (manifest.Object, error)
}

func newDiagnosticsProvider(deprecated deprecatedProvider, objects diagnosticsObjectsProvider) *diagnosticsProvider {
...
type diagnosticsProvider struct {
	deprecated deprecatedProvider
	objects    diagnosticsObjectsProvider
}

func (d diagnosticsProvider) DiagnoseFile(ctx context.Context, file *virtualFile) []messages.Diagnostic {
	if file.Err != nil {
	...
	for _, object := range file.FastAST {
		diagnostics = append(diagnostics, d.checkDeprecated(object)...)
	}
	return diagnostics
}

func (d diagnosticsProvider) validateObject(ctx context.Context, object *objectNode) []messages.Diagnostic {
	var diagnostics []messages.Diagnostic
...
func (d diagnosticsProvider) checkReferencedObjects(ctx context.Context, object *objectNode) []messages.Diagnostic {
...
func (d diagnosticsProvider) checkForProjectExistence(ctx context.Context, object *objectNode) []messages.Diagnostic {
...
func (d diagnosticsProvider) checkObjectExistence(
	ctx context.Context,
	node *yamlast.Node,
	propertyPath string,
	kind manifest.Kind,
...
func (d diagnosticsProvider) checkDeprecated(object *fastObjectNode) []messages.Diagnostic {
	...
	for _, path := range paths {
		for i, line := range object.Doc.Lines {
			...
			end := strings.Index(line.Value, ":")
			if end == -1 {
				end = len(line.Value)
			}
			diagnostics = append(diagnostics, messages.Diagnostic{
				Range:    newLineRange(object.Doc.Offset+i+1, line.Indent, end),
